...
const DocumentCell = ({ value, onChange, docType, projectId, projectDocuments, onUploadSuccess, isEditing = true }: any) => {
  ...
  if (!isEditing) {
    const selectedDoc = value ? projectDocuments.find((d: any) => d.id === value) : null;
    if (value && selectedDoc) {
      return (
        <div className="flex items-center justify-center p-2">
          <button onClick={() => handleView(selectedDoc.id, selectedDoc.fileName)} className="text-sky-600 hover:underline text-[10px] text-center max-w-[100px] truncate" title={selectedDoc.fileName}>
...
export default function SubkonPage() {
  ...
  const fetchSubkonData = async (projectId: string) => {
    try {
      const res = await api.get(`/project-subkons/${projectId}`);
  ...
  const handleSaveRow = async (subIdx: number) => {
    ...
    setSaving(true);
    try {
      const subkon = subkonData[subIdx];
      if (subkon.id) {
  ...
  const handleSave = async () => {
    ...
    try {
      const promises = subkonData.map(async (subkon) => {
        if (subkon.id) {
          return api.put(`/project-subkons/${subkon.id}`, subkon);
        } else {
          return api.post(`/project-subkons/${selectedProjectId}`, subkon);
        }
      });
      await Promise.all(promises);
      alert('Data Subkon berhasil disimpan!');
      fetchSubkonData(selectedProjectId);
    } catch (error: any) {
      console.error(error);
      alert(error.response?.data?.message || 'Terjadi kesalahan saat menyimpan data.');
  ...
  return (
    <div className="p-6 h-screen flex flex-col bg-slate-50 dark:bg-slate-900">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Data Subkon</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola data dan dokumen terkait subkontraktor (Parent-Child)</p>
        </div>
        <div className="flex space-x-3 bg-white dark:bg-slate-800 p-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <select 
            className="px-4 py-2 border rounded-lg bg-white dark:bg-slate-800 text-sm font-medium"
          ...
          >
            <Plus className="w-4 h-4 mr-1" /> Tambah Subkon Baru
          </button>
          <button
            onClick={handleSave}
      ...
      <div className="flex-1 overflow-auto smooth-scroll scroll-smooth border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 shadow-sm">
        <table className="w-full text-sm text-left whitespace-nowrap border-separate border-spacing-0">
          <thead className="text-[10px] text-center uppercase bg-slate-100 dark:bg-slate-900 sticky top-0 z-10 shadow-sm">
            <tr>
              {/* Induk Header */}
              <th className="border-l border-r border-t border-b border-slate-400 dark:border-slate-600 p-2 align-middle">Dropdown Subkon</th>
              <th className="border-r border-t border-b border-slate-400 dark:border-slate-600 p-2 align-middle">Nama Pekerjaan</th>
              <th className="border-r border-t border-b border-slate-400 dark:border-slate-600 p-2 align-middle">Editing Folder<br/><span className="text-slate-400 normal-case">Kategori (JASA/MAT)</span></th>
            ...
            {subkonData.length === 0 ? (
              <tr>
                <td colSpan={24} className="p-8 text-center text-slate-500">
                  {selectedProjectId ? 'Belum ada data subkon. Klik "Tambah Subkon Baru" untuk memulai.' : 'Silakan pilih proyek terlebih dahulu.'}
                </td>
              </tr>
            ) : (
              subkonData.map((subkon, subIdx) => {
                const termins = subkon.termins || [];
                const rowSpan = Math.max(1, termins.length);

                return termins.map((termin, termIdx) => {
                  const isFirstRow = termIdx === 0;

                  return (
                    <tr key={`${subIdx}-${termIdx}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      {/* --- INDUK COLUMNS (Hanya muncul di baris pertama termin) --- */}
                      {isFirstRow && (
                        <td className="border border-slate-300 dark:border-slate-600 p-0 align-top bg-white dark:bg-slate-900" rowSpan={rowSpan}>
                          {editingSubIdx === subIdx ? (
                            <select 
                              ...
                              className="w-full h-full min-w-[150px] px-2 py-2 text-xs border-0 bg-transparent focus:ring-2 focus:ring-sky-500 outline-none"
                            >
                              <option value="">-- Pilih --</option>
                              {masterSubkons.map(ms => (
                                <option key={ms.id} value={ms.id}>{ms.name}</option>
                              ))}
                            </select>
                          ) : (
                            <div className="p-2 text-xs min-w-[150px] font-semibold text-slate-700">{masterSubkons.find(m => m.id === subkon.masterSubkonId)?.name || '-'}</div>
                          )}
                        </td>
                      )}
                      {isFirstRow && (
                        <td className="border border-slate-300 dark:border-slate-600 p-0 align-top bg-white dark:bg-slate-900" rowSpan={rowSpan}>
                          {editingSubIdx === subIdx ? (
                            <textarea
                              value={subkon.namaPekerjaan || ''}
                              onChange={(e) => handleChangeSubkon(subIdx, 'namaPekerjaan', e.target.value)}
                              placeholder="Nama Pekerjaan..."
                              className="w-full h-full min-h-[60px] p-2 text-xs border-0 bg-transparent resize-y focus:ring-2 focus:ring-sky-500 outline-none"
                            />
                          ) : (
                            <div className="p-2 text-xs whitespace-pre-wrap">{subkon.namaPekerjaan || '-'}</div>
                          )}
                        </td>
                      )}
                      {isFirstRow && (
                        <td className="border border-slate-300 dark:border-slate-600 p-0 align-top bg-white dark:bg-slate-900" rowSpan={rowSpan}>
                          {editingSubIdx === subIdx ? (
                            <textarea
                              value={subkon.kategori || ''}
                              onChange={(e) => handleChangeSubkon(subIdx, 'kategori', e.target.value)}
                              placeholder="SPK-SUBCON/C..."
                              className="w-full h-full min-h-[60px] p-2 text-xs border-0 bg-transparent resize-y focus:ring-2 focus:ring-sky-500 outline-none font-mono"
                            />
                          ) : (
                            <div className="p-2 text-xs font-mono">{subkon.kategori || '-'}</div>
                          )}
                        </td>
                      )}
                      {isFirstRow && (
                        <td className="border border-slate-300 dark:border-slate-600 p-0 align-top bg-white dark:bg-slate-900" rowSpan={rowSpan}>
                          {editingSubIdx === subIdx ? (
                            ...
                            <div className="p-2 text-xs text-right whitespace-nowrap font-medium text-slate-800">{subkon.nilaiKontrak ? Number(subkon.nilaiKontrak).toLocaleString('id-ID') : '-'}</div>
                          )}
                        </td>
                      )}
                      ...
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-blue-50/30 dark:bg-blue-900/10">
                        {editingSubIdx === subIdx ? (
                          ...
                          <div className="p-2 text-xs text-right whitespace-nowrap">{termin.nilaiJasa ? Number(termin.nilaiJasa).toLocaleString('id-ID') : '-'}</div>
                        )}
                      </td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-blue-50/30 dark:bg-blue-900/10">
                        {editingSubIdx === subIdx ? (
                          ...
                          <div className="p-2 text-xs text-center">{termin.pembayaranPersen ? termin.pembayaranPersen + '%' : '-'}</div>
                        )}
                      </td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-blue-50/30 dark:bg-blue-900/10">
                        {editingSubIdx === subIdx ? (
                          ...
                          <div className="p-2 text-xs text-center">{termin.prosedurPenagihan || '-'}</div>
                        )}
                      </td>
                      ...
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">
                        {editingSubIdx === subIdx ? (
                            ...
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.bapp || '-'}</div>
                          )}
                      </td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">
                        <DocumentCell 
                      ...
                      {/* Manual Fields */}
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.laporanProgress} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'laporanProgress', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.laporanProgress || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.suratJalan} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'suratJalan', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.suratJalan || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.ceklist} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'ceklist', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.ceklist || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.bastBasT2} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'bastBasT2', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.bastBasT2 || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.proformaInvoice} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'proformaInvoice', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.proformaInvoice || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            ...
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.tandaTerimaTukarFaktur || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.invoice} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'invoice', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.invoice || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            <CellInput value={termin.kwitansi} onChange={(val: any) => handleChangeTermin(subIdx, termIdx, 'kwitansi', val)} />
                          ) : (
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.kwitansi || '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            ...
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.tanggalPengajuan ? new Date(termin.tanggalPengajuan).toLocaleDateString('id-ID') : '-'}</div>
                          )}</td>
                      <td className="border border-slate-300 dark:border-slate-600 p-0 bg-white dark:bg-slate-900">{editingSubIdx === subIdx ? (
                            ...
                            <div className="p-2 text-[10px] text-center whitespace-nowrap">{termin.tanggalDibayar ? new Date(termin.tanggalDibayar).toLocaleDateString('id-ID') : '-'}</div>
                          )}</td>
