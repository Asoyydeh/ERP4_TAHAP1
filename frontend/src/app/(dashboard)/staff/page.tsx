...
export default function StaffPage() {
  ...
  // Allow only managers (main users) from these specific roles
  ...
  const fetchStaffs = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Gagal mengambil data staf.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center max-w-md mx-auto">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
          Maaf, halaman ini hanya dapat diakses oleh Manajer Utama divisi.
        </p>
        <button
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'Gagal mendaftarkan staf baru.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Manajemen Staf Tim</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Daftarkan akun untuk staf bawahan Anda di divisi {user?.role}.</p>
        </div>
        {!user?.managerId && (
          <button
        ...
        <div className="overflow-x-auto">
          {staffs.length > 0 ? (
            <table className="min-w-full divide-y divide-slate-100">
              <thead>
                <tr className="text-left text-2xs font-semibold text-slate-400 uppercase tracking-wider bg-slate-50 dark:bg-slate-900/50">
                  <th className="py-3.5 px-6 rounded-l-lg">Nama Staf</th>
                  <th className="py-3.5 px-6">Email</th>
                  <th className="py-3.5 px-6 rounded-r-lg">Hak Akses Role</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-sm text-slate-600 dark:text-slate-300">
                {staffs.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:bg-slate-900/30 transition-all">
                    <td className="py-4 px-6 font-semibold text-slate-800 dark:text-slate-100">
                      <div className="flex items-center space-x-2">
                        <div className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 dark:text-slate-300">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-2xs font-bold border ${
                        !item.managerId 
                          ...
                          : 'bg-sky-50 text-sky-700 border-sky-100 dark:bg-sky-950 dark:text-sky-300'
                      }`}>
                        {item.role} {!item.managerId ? '(Divisi Utama)' : '(Staff)'}
                      </span>
                    </td>
                  </tr>
                ))}
            ...
            <div className="py-16 text-center text-sm text-slate-400">
              Belum ada staf terdaftar di tim Anda.
            </div>
          )}
        </div>
      </div>
